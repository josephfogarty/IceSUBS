...
rad_net = 400.0+lw_in*(1-eps_ice)-eps_ice*sigma*(T_it**4)
...
a0 = 400.0+lw_in*(1-eps_ice)+(rho_a*u_star_top)*((c_pa*c_h*air_temp(0.0)) \
...
for i in range(0,nt):
    ...
    a0 = 400.0+lw_in*(1-eps_ice)+(rho_a*u_star_top)*((c_pa*c_h*air_temp(i*dt)) \
                +(Ls*c_h*(q_a-ice_q(float(u[0])))))+(kappa_ice*(float(u[1]))/dx);
    def top_ice_flux(x):
        return a0 + a1*x + a4*(x**4)
